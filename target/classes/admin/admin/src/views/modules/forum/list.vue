	...
	<div class="main-content">
		<el-dialog title="帖子回复详情" :visible.sync="forumReplyDialogVisible">
			...
			<div class="demo-input-suffix">
				...
				<el-input v-model="forumContent" :disabled="true" placeholder="帖子内容" style="width: 80%" type="textarea">
				</el-input>
			</div>
		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '店长姓名' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.dianzhangName" placeholder="店长姓名"
							clearable></el-input>
					</el-form-item>

					<el-form-item :label="contents.inputTitle == 1 ? '教练姓名' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.jiaolianName" placeholder="教练姓名"
							clearable></el-input>
					</el-form-item>
				...
				<el-row class="ad"
					...
					<el-form-item>
						...
						<a style="text-decoration:none" class="el-button el-button--success"
							...
							href="http://localhost:8080/jianshenfanghuiyuan/upload/forumMuBan.xls">批量导入论坛数据模板</a>
						&nbsp;
						<el-upload v-if="isAuth('forum','导入导出')" style="display: inline-block"
							action="jianshenfanghuiyuan/file/upload" :on-success="forumUploadSuccess"
							:on-error="forumUploadError" :show-file-list=false>
							<el-button v-if="isAuth('forum','导入导出')" type="success" icon="el-icon-upload2">批量导入论坛数据
							</el-button>
						</el-upload>
						&nbsp;
						<!-- 导出excel -->
						...
						<template slot-scope="scope">
							...
							<span v-if="scope.row.dianzhangId">
								<div v-if="scope.row.dianzhangPhoto">
									<img :src="scope.row.dianzhangPhoto" width="100" height="100">
								</div>
								<div v-else>无图片</div>
							</span>
							<span v-if="scope.row.usersId">
				...
				_this.$http({
					...
					if (data && data.code === 0) {
						...
						data.data.list.forEach(el => {
							...
							forum.yonghuJifen = el.yonghuJifen;
				...
				_this.$http({
					...
					if (data && data.code === 0) {
						_this.$message({
							...
							duration: 1500,
							onClose: () => {
								_this.openReplyForum(_this.superIds, _this.forumTitle, _this
									.forumContent);
							}
						});
					} else {
