...
public class KechengCommentbackController {
    ...
    */
    ...
    public R update(@RequestBody KechengCommentbackEntity kechengCommentback, HttpServletRequest request){
...
//            kechengCommentback.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<KechengCommentbackEntity> queryWrapper = new EntityWrapper<KechengCommentbackEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        KechengCommentbackEntity kechengCommentbackEntity = kechengCommentbackService.selectOne(queryWrapper);
        kechengCommentback.setUpdateTime(new Date());
        if(kechengCommentbackEntity==null){
